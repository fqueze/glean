...
mod pings {
    ...
    pub static one: Lazy<PingType> = Lazy::new(|| {
        glean::private::PingType::new("one", true, true, true, true, false, vec![], vec![])
            .with_follows(false)
    });

    #[allow(non_upper_case_globals)]
    pub static two: Lazy<PingType> = Lazy::new(|| {
        glean::private::PingType::new("two", true, true, true, true, false, vec![], vec![])
            .with_follows(false)
    });
}
...
fn main() {
    ...
    match state {
        State::One => {
            glean::set_ping_enabled("one", true);
            glean::set_ping_enabled("two", false);
        }
        State::Two => {
            glean::set_ping_enabled("one", false);
            glean::set_ping_enabled("two", true);
        }
        State::Both => {
            glean::set_ping_enabled("one", true);
            glean::set_ping_enabled("two", true);
